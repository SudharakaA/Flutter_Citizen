...
import 'package:citizen_care_system/admin_pages/internal_services/postal_management/postal_tasks/postal_process_filter_page.dart';
...
class PostalProcessDetails extends StatefulWidget {
  const PostalProcessDetails({super.key});

  @override
  _PostalProcessDetailsPageState createState() =>
      _PostalProcessDetailsPageState();
}

class _PostalProcessDetailsPageState extends State<PostalProcessDetails> {
  String _searchQuery = '';
  ...
  List<Map<String, String>> itemsData = List.generate(
    30,
    (index) => {
      'Created': '2024-05-${(index % 30 + 1).toString().padLeft(2, '0')}',
      'Medium': index % 3 == 0
          ? 'Sinhala'
          : index % 3 == 1
              ? 'English'
              : 'Tamil',
      'Request Title': 'Title ${(index)}',
      'Request From': 'Citizen ${(index)}',
      'Assign From': 'Citizen ${(30 - index)}',
      'Assigned To': 'Citizen ${(index % 4)}',
      'Status': index % 3 == 0
          ? 'Delivered'
          : index % 3 == 1
              ? 'Pending'
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: const [
                      ...
                      DataColumn(label: Text("Request From")),
                      DataColumn(label: Text("Assign From")),
                    ...
                    rows: _paginatedItems.map((item) {
                      return DataRow(cells: [
                        ...
                        DataCell(Text(item['Medium']!)),
                        DataCell(Text(item['Request Title']!)),
                        DataCell(Text(item['Request From']!)),
                        DataCell(Text(item['Assign From']!)),
                        DataCell(Text(item['Assigned To']!)),
          ...
          HoverTapButton(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => const PostalProcessFilterPage()),
              );
            },
          ),
          HoverTapButton(
