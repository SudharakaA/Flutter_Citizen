...
import '../instructionServicesPages/traning_instruction_page.dart';
import 'package:http/http.dart' as http;
import 'package:easy_localization/easy_localization.dart';

class TrainingServicePage extends StatefulWidget {
  final String accessToken;
  final String citizenCode;

  const TrainingServicePage(
      {super.key, required this.accessToken, required this.citizenCode});

  @override
  State<TrainingServicePage> createState() => _TrainingServicePageState();
}

class _TrainingServicePageState extends State<TrainingServicePage> {
  late Future<List<Map<String, dynamic>>> _trainingServiceFuture;

  @override
  void initState() {
    super.initState();
    _trainingServiceFuture = fetchTrainingServices();
  }

  Future<List<Map<String, dynamic>>> fetchTrainingServices() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('failed_training_services'.tr());
    }
  }
  ...
  void navigateToInstructions(
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TraningInstructionPage(
